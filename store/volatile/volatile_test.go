	...
	for i := range numGoroutines {
		...
		go func(id int) {
			...
			if id < numGoroutines-1 {
				if _, err := xos.Stat(path); os.IsNotExist(err) {
...
func TestVolatile_AcquireAfterFullRelease(t *testing.T) {
	...
	path1, release1, err := store.Acquire(content)
	if err != nil {
	...
	path2, release2, err := store.Acquire(content)
	if err != nil {
...
func TestVolatile_ContentAddressed(t *testing.T) {
	...
	path1, release1, err := store.Acquire(content1)
	if err != nil {
	...
	path2, release2, err := store.Acquire(content2)
	if err != nil {
	...
	path3, release3, err := store.Acquire(content3)
	if err != nil {
...
func TestVolatile_DifferentAlgorithms(t *testing.T) {
	...
	for _, alg := range algorithms {
		store := volatile.New(filepath.Join(root, string(alg)), alg)
...
func TestVolatile_AlgorithmConsistency(t *testing.T) {
	...
	volatile1 := volatile.New(root, digest.SHA512)
	volatile2 := volatile.New(root, digest.SHA512)
