...
func TestCache_WriteDigestMismatch(t *testing.T) {
	...
	reader, writer, err := cache.Acquire(wrongDigest)
	if err != nil {
		t.Fatalf("Acquire() error: %v", err)
	}
...
func TestCache_WriteAlreadyExists(t *testing.T) {
	...
	reader1, writer1, _ := cache.Acquire(digest)
	...
	reader2, writer2, err := cache.Acquire(digest)
	if err != nil {
...
func TestCache_LargeContent(t *testing.T) {
	...
	// Write
	reader1, writer1, err := cache.Acquire(digest)
	if err != nil {
		t.Fatalf("Acquire() error: %v", err)
	}
...
func TestCache_WriterAbandonmentNoWrite(t *testing.T) {
	...
	reader, writer, err := cache.Acquire(emptyDigest)
	if err != nil {
	...
	reader, writer, err = cache.Acquire(nonEmptyDigest)
	if err != nil {
...
func TestCache_MultipleConcurrentDigests(t *testing.T) {
	...
	digests := make([]string, numDigests)
	...
	for i := range numDigests {
		...
		go func(idx int) {
			...
			reader, writer, err := cache.Acquire(digests[idx])
			if err != nil {
	...
	for i := range numDigests {
		reader, writer, err := cache.Acquire(digests[i])
		if err != nil {
