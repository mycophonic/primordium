...
func TestReadSeeker_ReadsAllData(t *testing.T) {
	...
	rs := iox.NewReadSeeker(bytes.NewReader(data))
...
func TestReadSeeker_SeekInvalidatesBuffer(t *testing.T) {
	...
	rs := iox.NewReadSeekerWithSize(bytes.NewReader(data), 64)
...
func TestReadSeeker_SeekCurrentBeyondBuffer(t *testing.T) {
	...
	data[100] = 'Y'
	rs := iox.NewReadSeekerWithSize(bytes.NewReader(data), 16)
...
func TestReadSeeker_CloseNonCloser(t *testing.T) {
	...
	rs := iox.NewReadSeeker(bytes.NewReader([]byte("data")))
	err := rs.Close()
